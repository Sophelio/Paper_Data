"""Dalia provider for the Lorenz attractor paper example.

Parquet files under ``data/``:

  * ``lorenz_dt001``                  — times, x, y, z
  * ``lorenz_dt001_exact_derivatives`` — times, x, y, z, dx, dy, dz

``dx``/``dy``/``dz`` are synthesized from the Lorenz vector field when a
file does not store them. Custom graphers live in ``custom_graphs.py``.
"""
...
DEFAULT_DATA_FOLDER = Path(r"D:\SIR_paper\Lorenz\data")
EXCLUDED_VARS = {"times"}
SIGMA = 10.0
RHO = 28.0
BETA = 8.0 / 3.0
...
def _ensure_requested_columns(df: pd.DataFrame, keys: list[str]) -> pd.DataFrame:
    ...
    if not missing:
        return df
    if any(k in {"dx", "dy", "dz"} for k in missing) and {"x", "y", "z"}.issubset(df.columns):
...
def get_provider(_: Any) -> dict:
    ...
    all_possible_signals = _discover_signals(data_folder) or ["x", "y", "z", "dx", "dy", "dz"]
