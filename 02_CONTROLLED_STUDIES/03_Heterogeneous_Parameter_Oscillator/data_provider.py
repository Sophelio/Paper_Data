"""Dalia provider for the stochastic harmonic-oscillator ensemble.

Each parquet under ``data/`` is one realization of
``h_alpha(x) = sin(alpha*pi*x)`` with columns:

    times, h, dh, d2h, alpha

``times`` is the spatial coordinate x; ``alpha`` is constant metadata.
``dh``/``d2h`` are synthesized from ``h`` and ``alpha`` when missing.
"""
...
DEFAULT_DATA_FOLDER = Path(r"D:\SIR_paper\Stochastic oscillator\data")
...
def _ensure_requested_columns(df: pd.DataFrame, keys: list[str]) -> pd.DataFrame:
    ...
    if not missing:
        return df
    if any(k in {"dh", "d2h"} for k in missing) and {"alpha", "times", "h"}.issubset(df.columns):
...
def get_provider(_: Any) -> dict:
    ...
    all_possible_signals = _discover_signals(data_folder) or ["h", "dh", "d2h"]
    ...
    return {
        ...
        "dataset_id": "stochastic_oscillator",
        ...
        "custom_grapher_dictionary": build_custom_graphers(),
    }
